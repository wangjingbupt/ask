...
class ViewItem {
	...
	public function render($datas)
	{
		...
		$html .="<form class='form-search' target='_self' method='GET'><input type='text' name = 'itemId' class='input-medium search-query' value='{$this->itemId}'> <button type='submit' class='btn'>Search</button></form>";
		...
		if(is_array($datas['post']) && !empty($datas['post']))
		{
				...
				$html .= <<<HTML
					...
					<tr>
					<td style='width:45%;'>货号：$id</br>
					状态：$status</br>
					价格：$myPrice</br>
					</td>
					<td style='width:55%;'><img src="$img"></td>
					</tr>
					</table>
					</div><!--/.well -->
